 ...
 * Picture it as a cycle in a directed graph: start from a node, go to the next,
 * next again, and so on until eventually coming back to the starting point.
 */
public interface Cycle<T> {
    
	/**
	 * {@link Cycle#where() Where} are we in the cycle? 
	 * We can either be at the {@link #Start beginning} or {@link #OnWay on the
    ...
    /**
     * Convenience method to combine a call to {@link #next() next} and {@link
     * Cycle#where() where}.
